...
void Retina(const std::string file, const std::string fileoutput, int iteration){
	...
	double soglia = 300;
	int count = 0;
	std::map<int, double> Zmap; //this must go into GeneralUtils package
		...
		for(auto ColumnElement: Grid){
			...
			for(auto element:ColumnElement){
				...
				*/
				...
				CellReset(element, TrackXPlaceholder, TrackYPlaceholder, VecIndex, Vec, qx, qy, mx, my, lineX, lineY);
				...
				// Check for closest point function for each X layer
				for(auto l:LayerX){
					for(int j = 0; j<e.ClusterPosizione.size(); j++){ //push all points close by 0.5 cm from road
					...
					if(VecIndex.size() !=0){ //if there are any points close by 0.5 cm
						n = XZ->GetN();
				...
				// Check for closest point function for each Y layer
				for(auto l:LayerY){
					for(int j = 0; j<e.ClusterPosizione.size(); j++ ){//push all points close by 0.5 cm from road
					...
					//If there are more than 3 points, make a fit and keep filling placeholder	
					if((XZ->GetN()>=3)){
						XZ->Fit("lineX","qEX0");
